...
@router.post("/dub/rpt-init/{job_id}")
async def rpt_init(job_id: str, request: Request):
    """Create RPT working copies of the dubbed video and audio
    if they don't already exist. Safe to call multiple times —
    returns immediately if copies already exist.
    """
    import shutil
    auth_header = request.headers.get("Authorization", "")
    token = auth_header.removeprefix("Bearer ").strip()
    ...
    # Detect the dubbed video file (dubbed_{lang}.mp4)
    candidates = [
    ...
    if not candidates:
        raise HTTPException(
            status_code=404,
            detail=f"No dubbed video found for job {job_id}"
        )

    master_video = os.path.join(output_dir, candidates[0])
    lang = candidates[0][len("dubbed_"):-len(".mp4")]
    rpt_video = os.path.join(output_dir, f"dubbed_rpt_{lang}.mp4")

    master_audio = os.path.join(output_dir, "dubbed_audio.wav")
    rpt_audio = os.path.join(output_dir, "rpt_audio.wav")

    created = []

    if not os.path.exists(rpt_video):
        await asyncio.to_thread(shutil.copy2, master_video, rpt_video)
        created.append("video")

    if os.path.exists(master_audio) and not os.path.exists(rpt_audio):
        await asyncio.to_thread(shutil.copy2, master_audio, rpt_audio)
        created.append("audio")

    status = "created" if created else "exists"
    logger.info(
        f"[RPT INIT] job={job_id} status={status} created={created}"
    )
    return {
        "job_id": job_id,
        "status": status,
        "rpt_video_url": f"/api/download-rpt/{job_id}/{lang}",
        "created": created,
    }


@router.get("/download-rpt/{job_id}/{lang}")
async def download_rpt(job_id: str, lang: str):
    """Serve the RPT working copy video."""
    rpt_path = os.path.join(settings.DUBBED_DIR, job_id, f"dubbed_rpt_{lang}.mp4")
    if not os.path.exists(rpt_path):
        raise HTTPException(status_code=404, detail="RPT video not found")
    return FileResponse(
        rpt_path,
        media_type="video/mp4",
